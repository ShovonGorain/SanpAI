        ...
        try:
            ...
            for i in range(len(clips) - 1):
                ...
                if transition_name == 'fade':
                    ...
                    final_clips[-1] = final_clips[-1].fx(comp_fadeout, transition_duration)
                    ...
                    final_clips.append(clip2.set_pos(('center', height)).fx(mp.vfx.scroll, h=height, w=width))

                elif transition_name == 'resize':
                    final_clips.append(clip2.fx(mp.vfx.resize, lambda t: 1 + t * 0.5))

                elif transition_name == 'zoom_pan':
                    # Ken Burns effect - zoom in and pan
                    final_clips.append(
                        clip2.fx(
                            mp.vfx.resize, lambda t: 1 + 0.2 * t
                        ).set_pos(
                            lambda t: (
                                'center',
                                'center'
            ...
            if audio_clip:
                try:
                    print(f"🔊 Attaching audio. Video duration: {video.duration:.2f}s, Audio duration: {audio_clip.duration:.2f}s")

                    # Trim or loop audio to match video duration
                    if audio_clip.duration < video.duration:
                        print("λού Audio is shorter than video, looping...")
                        audio_clip = audio_clip.loop(duration=video.duration)
                    elif audio_clip.duration > video.duration:
                        print("✂️ Audio is longer than video, trimming...")
